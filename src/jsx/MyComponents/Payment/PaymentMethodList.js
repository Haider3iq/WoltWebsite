...
export default function PaymentMethodList({rest,aditionalCosts, activeNumber, setPaymentName}) {
  ...
  const fakeProfileImage = "https://burgerking.fi/wp-content/uploads/BK_Pride_kadet_1200x800-1024x683.jpg";
	...
	useEffect(() => {
		if(auth?.prefferedPayment) {
			setActiveIcon(data.map(it => it.id).indexOf(auth.prefferedPayment) + 1) 
		}
	}, [auth?.preferredPayment])

	useEffect(() => {

        if(activeNumber) {
            setActiveIcon(activeNumber)
        }
    }, [activeNumber])
  ...
  return (
   ...
   {data.map((data, index) => {
                      ...
                      return (
                        <div onClick={() => onClick(index + 1)} class={activeIcon === index + 1?"form__radiosActive" : "form__radios"} key={index}>
                            <div class="form__radio">
                                <div className="formUnderDiv" for={data.id}>
                                    <svg class="customIcon">
                                    <use xlinkHref={data.xlinkHref} />
                                    </svg>
								...
								{activeIcon === index + 1 ? <IoCheckmarkCircle className="paymentMethod"/> :
									 <IoEllipse className="paymentMethod"/>}

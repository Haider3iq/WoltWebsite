import { setVendorInfo, setVendorCommodities } from "../../store/actions/AuthActions"
...
export async function fetch_vendor_request (myId,) {

    // const dispatch = Store.dispatch

    const function_name = "fetch_vendor_request"
    // console.log(function_name)
    try{
      await fetch('http://localhost:5000/fetchVendor', {
      method: "POST",
      headers: {
    ...
    .then((responseJson) => {
      ...
      if(responseJson.empty) {
          console.log("empty was returned")
      } else {
          // console.log(`responsejson: ${JSON.stringify(responseJson[0])}`)
        //   setvendor(responseJson.activevendor ? responseJson.activevendor : [])
        // console.log("responseJson:", responseJson.activevendor)
        store.dispatch(
            setVendorInfo(responseJson.vendorInfo ? responseJson.vendorInfo : [{"empty": "empty"}])
            );

            // console.log("lastDelivery", responseJson.lastDelivery)
        store.dispatch(
            setVendorCommodities(responseJson.vendorCommodities ? responseJson.vendorCommodities : [{"empty": "empty"}])
            );

            console.log("responseJson.vendorCommodities", responseJson.vendorCommodities)
            console.log("responseJson.vendorInfo", responseJson.vendorInfo)
...
// export async function fetch_messages_request (myId){
...
// export async function fetch_payouts_request (myId){
...
// export async function addNewMessage(textMessage, riderID, messageId, createdAt, messages, sent){
...
//             let data = tasks
//             if(tasks ) {
//               console.log("vendor ids before",data?.map(it => it.id))
//               var foundIndex = data?.findIndex(it => it.id == task.id);
//               data[foundIndex] = task;
//               // console.log("vendor ids",data?.map(it => it.id))
//               // setvendor(vendor.map(it => it.id !== task.id ? it : task))
//               Store.dispatch(setRiderTasks(data))
//               // setActive(active === "second" ? "first" : active === "first" ? "second" : "third")
//               // console.log("vendor: ", deliveries, "active is: ", active)
//             }
...
//   Store.dispatch(setVendorCommodities(task))
...
//   locationNow, workingStatus, onlineStatus, email, password
//   ) {
...
// export async function fetch_new_task_request (myId) {
